export const site = {
  name: 'Apurva Mukherjee',
  title: 'Apurva Mukherjee | Portfolio',
  greeting: "Namaste(); I'm",
  tagline: 'I design & code for web.',
  summary:
    'I build production-grade apps end-to-end — from system design to deployment. Currently shipping at Mind Webs Ventures.',
  contactHref: 'https://www.instagram.com/sameapurva',
  resumeHref: '/assets/resume/ApurvaMukherjee.pdf',
  footerQuote: 'Learning, Living, and Leveling Up.',
  footerCopyright: 'Design & Built by Apurva Mukherjee © twentytwentysix',
} as const
